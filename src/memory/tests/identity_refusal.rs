...
// ---- Role-aware engine methods (issue #220) ----------------------------
//
// The public API is the four role-aware methods, all delegating to the
// engine's private role helper. These model-free tests pin the delegation
// through the store's `#[cfg(test)]` embedder dispatch (the allowed second,
// test-only prefix site): the recording embedder receives exactly
// `role.prefix(profile) + content` for the store's model, so the role-aware
// entry points and the test-embedder contract cannot drift apart. The
// engine's own pure `prefix_input` helper is pinned in
// `src/embedding_tests.rs`.
